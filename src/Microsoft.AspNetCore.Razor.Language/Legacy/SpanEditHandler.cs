...
namespace Microsoft.AspNetCore.Razor.Language.Legacy
{
    internal class SpanEditHandler
    {
        ...
        {
            var newContent = ApplyChange(change, target.Content, target.Start.AbsoluteIndex);
        ...
        public string ApplyChange(SourceChange change, string content, int changeOffset)
        {
            var changeRelativePosition = change.Span.AbsoluteIndex - changeOffset;

            Debug.Assert(changeRelativePosition >= 0);
            return content.Remove(changeRelativePosition, change.Span.Length).Insert(changeRelativePosition, change.NewText);
        }
